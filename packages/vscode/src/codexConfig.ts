...
const CODEX_CONFIG_DIR = CODEX_HOME;
const AGENT_DIR = path.join(CODEX_CONFIG_DIR, 'agents');
const COMMAND_DIR = path.join(CODEX_CONFIG_DIR, 'commands');
const GLOBAL_SNIPPET_DIR = path.join(CODEX_CONFIG_DIR, 'snippet');
const GLOBAL_SNIPPET_DIR_ALT = path.join(CODEX_CONFIG_DIR, 'snippets');
const CONFIG_FILE = path.join(CODEX_CONFIG_DIR, 'config.json');
const CUSTOM_CONFIG_FILE = process.env.OPENCHAMBER_CODEX_CONFIG
  ? path.resolve(process.env.OPENCHAMBER_CODEX_CONFIG)
  : null;
...
const ensureProjectAgentDir = (workingDirectory: string): string => {
  const projectAgentDir = path.join(workingDirectory, '.codex', 'agents');
  if (!fs.existsSync(projectAgentDir)) {
    fs.mkdirSync(projectAgentDir, { recursive: true });
  }
  const legacyProjectAgentDir = path.join(workingDirectory, '.codex', 'agent');
...
const getProjectAgentPath = (workingDirectory: string, agentName: string): string => {
  const pluralPath = path.join(workingDirectory, '.codex', 'agents', `${agentName}.md`);
  const legacyPath = path.join(workingDirectory, '.codex', 'agent', `${agentName}.md`);
...
const getUserAgentPath = (agentName: string, lookupCache: AgentLookupCache = globalAgentLookupCache): string => {
  ...
  const legacyPath = path.join(CODEX_CONFIG_DIR, 'agent', `${agentName}.md`);
...
const ensureProjectCommandDir = (workingDirectory: string): string => {
  const projectCommandDir = path.join(workingDirectory, '.codex', 'commands');
  if (!fs.existsSync(projectCommandDir)) {
    fs.mkdirSync(projectCommandDir, { recursive: true });
  }
  const legacyProjectCommandDir = path.join(workingDirectory, '.codex', 'command');
...
const getProjectCommandPath = (workingDirectory: string, commandName: string): string => {
  const pluralPath = path.join(workingDirectory, '.codex', 'commands', `${commandName}.md`);
  const legacyPath = path.join(workingDirectory, '.codex', 'command', `${commandName}.md`);
  if (fs.existsSync(legacyPath) && !fs.existsSync(pluralPath)) return legacyPath;
  return pluralPath;
};
...
const getUserCommandPath = (commandName: string): string => {
  ...
  const legacyPath = path.join(CODEX_CONFIG_DIR, 'command', `${commandName}.md`);
  if (fs.existsSync(legacyPath) && !fs.existsSync(pluralPath)) return legacyPath;
  return pluralPath;
};
...
const getProjectSnippetDirs = (workingDirectory?: string): Array<{ dir: string; source: SnippetScope }> => {
  ...
  return [
    { dir: path.join(workingDirectory, '.codex', 'snippets'), source: 'project' },
    { dir: path.join(workingDirectory, '.codex', 'snippet'), source: 'project' },
  ];
};
...
const getWritableSnippetDir = (scope: SnippetScope, workingDirectory?: string): string => {
  if (scope === 'project') {
    ...
    const preferred = path.join(workingDirectory, '.codex', 'snippet');
    const alternate = path.join(workingDirectory, '.codex', 'snippets');
...
const resolvePromptFilePath = (reference: string): string | null => {
  ...
  if (target.startsWith('./')) {
    target = path.join(CODEX_CONFIG_DIR, target.slice(2));
  } else if (!path.isAbsolute(target)) {
    target = path.join(CODEX_CONFIG_DIR, target);
  }
 ...
 * Priority: root > .codex/, json > jsonc
 */
const getProjectConfigCandidates = (workingDirectory?: string): string[] => {
  if (!workingDirectory) return [];
  return [
    path.join(workingDirectory, 'codex.json'),
    path.join(workingDirectory, 'codex.jsonc'),
    path.join(workingDirectory, '.codex', 'codex.json'),
    path.join(workingDirectory, '.codex', 'codex.jsonc'),
  ];
};
...
const getProjectConfigPath = (workingDirectory?: string): string | null => {
  ...
  // Default to root codex.json for new configs
  return candidates[0] || null;
};

const getConfigPaths = (workingDirectory?: string) => ({
  userPaths: [
    path.join(CODEX_CONFIG_DIR, 'config.json'),
    path.join(CODEX_CONFIG_DIR, 'codex.json'),
    path.join(CODEX_CONFIG_DIR, 'codex.jsonc'),
  ],
...
const resolveSkillSearchDirectories = (workingDirectory?: string): string[] => {
  ...
  pushDir(path.join(os.homedir(), '.codex'));
  pushDir(process.env.CODEX_CONFIG_DIR ? path.resolve(process.env.CODEX_CONFIG_DIR) : null);
...
const getActiveCustomConfigPath = (): string | null =>
  process.env.OPENCHAMBER_CODEX_CONFIG ? path.resolve(process.env.OPENCHAMBER_CODEX_CONFIG) : null;

const getActiveOpencodeConfigDir = (): string => {
  const customConfigPath = getActiveCustomConfigPath();
  return customConfigPath ? path.dirname(customConfigPath) : CODEX_CONFIG_DIR;
};
...
const getActiveUserConfigPaths = (): string[] => {
  ...
  return [
    ...
    path.join(configDir, 'codex.jsonc'),
  ];
};
...
const ensureProjectPluginConfigPath = (workingDirectory?: string | null): string => {
  ...
  return path.join(workingDirectory, '.codex', 'codex.json');
};
...
const getPluginDir = (scope: PluginScope, workingDirectory?: string | null): string => {
  if (scope === 'project') {
    if (!workingDirectory) throw codedError('Project plugin scope requires working directory', 'INVALID_SCOPE');
    return path.join(workingDirectory, '.codex', 'plugins');
  }
...
const ensureProjectMcpConfigPath = (workingDirectory: string): string => {
  const projectConfigDir = path.join(workingDirectory, '.codex');
  if (!fs.existsSync(projectConfigDir)) {
    fs.mkdirSync(projectConfigDir, { recursive: true });
  }
  return path.join(projectConfigDir, 'codex.json');
};
...
const writeMdFile = (filePath: string, frontmatter: Record<string, unknown>, body: string) => {
  // Filter out null/undefined values - Codex expects keys to be omitted rather than set to null
...
export const createAgent = (agentName: string, config: Record<string, unknown>, workingDirectory?: string, scope?: AgentScope) => {
  ...
  const layers = readConfigLayers(workingDirectory);
  const jsonSource = getJsonEntrySource(layers, 'agent', agentName);
  if (jsonSource.exists) throw new Error(`Agent ${agentName} already exists in codex.json`);
...
export const updateAgent = (agentName: string, updates: Record<string, unknown>, workingDirectory?: string) => {
  ...
  // Check if agent exists in codex.json across all config layers
  const layers = readConfigLayers(workingDirectory);
  const jsonSource = getJsonEntrySource(layers, 'agent', agentName);
...
export const createCommand = (commandName: string, config: Record<string, unknown>, workingDirectory?: string, scope?: CommandScope) => {
  ...
  const layers = readConfigLayers(workingDirectory);
  const jsonSource = getJsonEntrySource(layers, 'command', commandName);
  if (jsonSource.exists) throw new Error(`Command ${commandName} already exists in codex.json`);
...
const SKILL_DIR = path.join(CODEX_CONFIG_DIR, 'skills');
...
const getUserSkillDir = (skillName: string): string => {
  ...
  const legacyPath = path.join(CODEX_CONFIG_DIR, 'skill', skillName);
  if (fs.existsSync(legacyPath) && !fs.existsSync(pluralPath)) return legacyPath;
  return pluralPath;
};
...
const getUserSkillPath = (skillName: string): string => {
  ...
  const legacyPath = path.join(CODEX_CONFIG_DIR, 'skill', skillName, 'SKILL.md');
  if (fs.existsSync(legacyPath) && !fs.existsSync(pluralPath)) return legacyPath;
  return pluralPath;
};

const getProjectSkillDir = (workingDirectory: string, skillName: string): string => {
  const pluralPath = path.join(workingDirectory, '.codex', 'skills', skillName);
  const legacyPath = path.join(workingDirectory, '.codex', 'skill', skillName);
  if (fs.existsSync(legacyPath) && !fs.existsSync(pluralPath)) return legacyPath;
  return pluralPath;
};

const getProjectSkillPath = (workingDirectory: string, skillName: string): string => {
  const pluralPath = path.join(workingDirectory, '.codex', 'skills', skillName, 'SKILL.md');
  const legacyPath = path.join(workingDirectory, '.codex', 'skill', skillName, 'SKILL.md');
  if (fs.existsSync(legacyPath) && !fs.existsSync(pluralPath)) return legacyPath;
  return pluralPath;
};
...
const getSkillScope = (skillName: string, workingDirectory?: string): {
  ...
  if (workingDirectory) {
    // Check .codex/skill first
    const projectPath = getProjectSkillPath(workingDirectory, skillName);
    if (fs.existsSync(projectPath)) {
      return { scope: SKILL_SCOPE.PROJECT, path: projectPath, source: 'codex' };
    }
  ...
  if (fs.existsSync(userPath)) {
    return { scope: SKILL_SCOPE.USER, path: userPath, source: 'codex' };
  }
...
export const discoverSkills = (workingDirectory?: string): DiscoveredSkill[] => {
  ...
  const homeOpencodeDir = path.resolve(path.join(os.homedir(), '.codex'));
  const customConfigDir = process.env.CODEX_CONFIG_DIR
    ? path.resolve(process.env.CODEX_CONFIG_DIR)
    : null;
  ...
  for (const skillPath of configuredPaths) {
    ...
    for (const skillMdPath of walkSkillMdFiles(resolved)) {
      addSkillFromMdFile(skills, skillMdPath, SKILL_SCOPE.PROJECT, 'codex');
    }
  }
  ...
  if (process.env.XDG_CACHE_HOME) {
    cacheCandidates.push(path.join(process.env.XDG_CACHE_HOME, 'codex', 'skills'));
  }
  cacheCandidates.push(path.join(os.homedir(), '.cache', 'codex', 'skills'));
  cacheCandidates.push(path.join(os.homedir(), 'Library', 'Caches', 'codex', 'skills'));
  ...
  for (const cacheRoot of cacheCandidates) {
    ...
    for (const entry of entries) {
      ...
      for (const skillMdPath of walkSkillMdFiles(skillRoot)) {
        addSkillFromMdFile(skills, skillMdPath, SKILL_SCOPE.USER, 'codex');
      }
    }
  }
...
export const getSkillSources = (
  ...
  if (isBuiltInDiscovered) {
    ...
    mdSource = matchedDiscovered?.source || 'codex';
...
export const createSkill = (skillName: string, config: Record<string, unknown>, workingDirectory?: string, scope?: SkillScope): void => {
  ...
  const requestedSource: SkillSource = config.source === 'agents' ? 'agents' : 'codex';
