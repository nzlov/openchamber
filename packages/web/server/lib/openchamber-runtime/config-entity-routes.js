export const registerConfigEntityRoutes = (app, dependencies) => {
  ...
  // Build the response for a config mutation based on whether the runtime
  // reloaded the change. When connected to an external runtime that OpenChamber
  // cannot restart, the change is persisted to disk but the running server will
  // not serve it until the user restarts that server. We must not
  ...
  const completeMcpMutation = async (res, action, name, applyChange) => {
    ...
    try {
      await refreshRuntimeAfterConfigChange(`mcp ${action}`);
      return res.json({
        success: true,
        requiresReload: true,
      ...
      return res.json({
        ...
        requiresReload: false,
        reloadFailed: true,
        message: `MCP server "${name}" ${action}d, but runtime reload failed.`,
        warning: error.message || 'Runtime reload failed after the MCP configuration changed',
      });
    }
  };
  ...
  app.post('/api/config/agents/:name', async (req, res) => {
    try {
      ...
      const refreshResult = await refreshRuntimeAfterConfigChange('agent creation', {
        agentName
      });

      res.json(buildConfigMutationResponse(refreshResult, {
        liveMessage: `Agent ${agentName} created successfully. Reloading interface…`,
        manualRestartMessage: `Agent ${agentName} saved. Restart your connected Codex runtime to apply the change.`,
      }));
    } catch (error) {
  ...
  app.patch('/api/config/agents/:name', async (req, res) => {
    try {
      ...
      const refreshResult = await refreshRuntimeAfterConfigChange('agent update');
  ...
  app.delete('/api/config/agents/:name', async (req, res) => {
    try {
      ...
      const refreshResult = await refreshRuntimeAfterConfigChange('agent deletion');

      res.json(buildConfigMutationResponse(refreshResult, {
        liveMessage: `Agent ${agentName} deleted successfully. Reloading interface…`,
        manualRestartMessage: `Agent ${agentName} deleted. Restart your connected Codex runtime to apply the change.`,
      }));
    } catch (error) {
  ...
  app.post('/api/config/commands/:name', async (req, res) => {
    try {
      ...
      await refreshRuntimeAfterConfigChange('command creation', {
        commandName
      });
  ...
  app.patch('/api/config/commands/:name', async (req, res) => {
    try {
      ...
      await refreshRuntimeAfterConfigChange('command update');
  ...
  app.delete('/api/config/commands/:name', async (req, res) => {
    try {
      ...
      await refreshRuntimeAfterConfigChange('command deletion');
