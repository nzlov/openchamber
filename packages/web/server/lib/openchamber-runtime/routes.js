...
export const registerRuntimeConfigRoutes = (app, dependencies) => {
  const {
    clientReloadDelayMs,
  ...
  app.get('/api/provider/:providerId/source', async (req, res) => {
    try {
      ...
      const headerDirectory = typeof req.get === 'function' ? req.get('x-codex-directory') : null;
  ...
  app.delete('/api/provider/:providerId/auth', async (req, res) => {
    try {
      ...
      if (removed) {
        await refreshRuntimeAfterConfigChange(`provider ${providerId} disconnected (${scope})`);
      }
  ...
  app.put('/api/behavior/agents-md', async (req, res) => {
    try {
      ...
      // Refresh the runtime so it picks up the new AGENTS.md without a full restart.
      try {
        await refreshRuntimeAfterConfigChange('global behavior (AGENTS.md) updated');
      } catch {
