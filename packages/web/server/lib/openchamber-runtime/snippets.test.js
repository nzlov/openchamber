...
describe('snippets', () => {
  ...
  test('loads project snippets with aliases and description', () => {
    writeSnippet('.codex/snippet/review.md', '---\naliases: [rev]\ndescription: Review helper\n---\nReview carefully.');
  ...
  test('snippet directory wins over snippets directory', () => {
    writeSnippet('.codex/snippets/same.md', 'Old');
    writeSnippet('.codex/snippet/same.md', 'New');

    expect(getSnippet('same', projectDir)?.content).toBe('New');
  });

  test('canonical snippet names take precedence over colliding aliases', () => {
    writeSnippet('.codex/snippets/review.md', 'Review by name');
    writeSnippet('.codex/snippet/helper.md', '---\naliases: [review, help]\n---\nReview by alias');
  ...
  test('canonical snippet names override earlier same-directory aliases', () => {
    writeSnippet('.codex/snippet/alias-first.md', '---\naliases: [review, assist]\n---\nReview by alias');
    writeSnippet('.codex/snippet/review.md', 'Review by name');

    const snippetDir = path.join(projectDir, '.codex', 'snippet');
  ...
  test('expands snippets recursively with prepend and append blocks', () => {
    writeSnippet('.codex/snippet/base.md', 'Base text');
    writeSnippet('.codex/snippet/review.md', '<prepend>Before</prepend>Review #base<append>After</append>');
