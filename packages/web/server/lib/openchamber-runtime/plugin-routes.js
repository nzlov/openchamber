...
export const registerPluginRoutes = (app, dependencies) => {
  ...
  const completePluginMutation = async (res, operation, _noun, applyChange) => {
    ...
    try {
      await refreshRuntimeAfterConfigChange(`plugin ${operation}`);
      ...
      return res.json({
        ...
        message: `Plugin ${pastTense}, but runtime reload failed.`,
        reloadDelayMs: clientReloadDelayMs,
        reloadFailed: true,
        warning: error.message || 'Runtime reload failed after plugin config changed',
      });
    }
  };
