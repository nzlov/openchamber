...
function ensureProjectSkillDir(workingDirectory) {
  const projectSkillDir = path.join(workingDirectory, '.codex', 'skills');
...
function getUserSkillPath(skillName) {
  return path.join(SKILL_DIR, skillName, 'SKILL.md');
}
...
function discoverSkills(workingDirectory) {
  ...
  const homeCodexDir = path.resolve(path.join(os.homedir(), '.codex'));
...
function createSkill(skillName, config, workingDirectory, scope) {
  ...
  const requestedSource = config?.source === 'agents' ? 'agents' : 'codex';
