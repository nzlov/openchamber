export const registerSkillRoutes = (app, dependencies) => {
  ...
  const inferSkillScopeAndSourceFromPath = (skillPath, workingDirectory) => {
    ...
    const isProjectScoped = projectAncestors.some((ancestor) => {
      const candidates = [
        path.join(ancestor, '.codex', 'skills'),
    ...
    const userRoots = [
      ...
      process.env.CODEX_CONFIG_DIR ? path.resolve(process.env.CODEX_CONFIG_DIR, 'skills') : null,
    ].filter(Boolean);
  ...
  const findDiscoveredSkill = (skillName, directory) =>
    discoverSkills(directory).find((skill) => skill.name === skillName) || null;
  ...
  app.get('/api/config/skills', async (req, res) => {
    try {
      ...
      const skills = mergeDiscoveredSkills([], localSkills);
  ...
  app.get('/api/config/skills/catalog/source', async (req, res) => {
    try {
      ...
      const resolvedDiscovered = mergeDiscoveredSkills([], discoverSkills(directory));
  ...
  app.post('/api/config/skills/install', async (req, res) => {
    try {
      ...
      if (isClawdHubSource(source)) {
        ...
        if (requiresReload) {
          await refreshRuntimeAfterConfigChange?.('skills install');
        }
  ...
  app.get('/api/config/skills/:name', async (req, res) => {
    try {
      ...
      const discoveredSkill = findDiscoveredSkill(skillName, directory);
  ...
  app.post('/api/config/skills/:name', async (req, res) => {
    try {
      ...
      await refreshRuntimeAfterConfigChange?.('skill creation');
  ...
  app.patch('/api/config/skills/:name', async (req, res) => {
    try {
      ...
      await refreshRuntimeAfterConfigChange?.('skill update');
  ...
  app.delete('/api/config/skills/:name', async (req, res) => {
    try {
      ...
      await refreshRuntimeAfterConfigChange?.('skill deletion');
