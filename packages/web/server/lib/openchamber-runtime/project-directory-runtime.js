...
const getDirectoryHint = (req) => {
  if (typeof req.get !== 'function') {
    return null;
  }
  const rawHeaderDirectory = req.get('x-openchamber-directory');
  const headerEncoding = req.get('x-openchamber-directory-encoding');
  return rawHeaderDirectory ? safeDecodeMarkedURIComponent(rawHeaderDirectory, headerEncoding) : null;
};
...
export const createProjectDirectoryRuntime = (dependencies) => {
  ...
  const resolveProjectDirectory = async (req) => {
    const headerDirectory = getDirectoryHint(req);
