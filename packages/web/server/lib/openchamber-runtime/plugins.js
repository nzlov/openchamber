...
function parsedKindForSpec(spec) {
  ...
  // Do NOT use `includes(path.sep)` because scoped npm packages legitimately contain '/'.
  return isPathSpec(spec) ? 'path' : 'npm';
}

function getActiveCodexConfigDir() {
  const customConfigPath = process.env.CODEX_CONFIG;
  if (customConfigPath) {
...
function getActiveUserConfigPaths() {
  const configDir = getActiveCodexConfigDir();
  return [
...
function getActiveCustomConfigPath() {
  return process.env.CODEX_CONFIG ? path.resolve(process.env.CODEX_CONFIG) : null;
}
...
function ensureProjectConfigPath(workingDirectory) {
  ...
  const configDir = path.join(workingDirectory, '.codex');
  fs.mkdirSync(configDir, { recursive: true });
  return path.join(configDir, 'codex.json');
}
...
function pluginDirForScope(scope, workingDirectory) {
  ...
  return path.join(getActiveCodexConfigDir(), 'plugins');
}
