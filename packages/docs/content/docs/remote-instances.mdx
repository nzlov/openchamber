...
> Remote instances are a **desktop-only** feature. On the web or in VS Code, run OpenChamber on the remote machine and connect to that OpenChamber URL instead.
...
- [Codex Runtime](/codex-server/) — configure the Codex CLI runtime
