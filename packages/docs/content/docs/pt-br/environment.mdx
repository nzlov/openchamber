...
description: Configure a integração do OpenChamber e do Codex com variáveis de ambiente.
---
...
Caminho para o executável `codex` que o OpenChamber deve usar.

### `CODEX_CONFIG`

Caminho para um arquivo de configuração específico do Codex.

### `CODEX_CONFIG_DIR`

Caminho para o diretório de configuração do Codex para agents, skills, snippets e descoberta de config.

### `CODEX_DATA_DIR`

Diretório de dados personalizado para o servidor Codex gerenciado.

### `CODEX_WSL_DISTRO`

Escolhe a distro WSL usada pela integração do Codex no Windows.

### `OPENCHAMBER_CODEX_WSL_DISTRO`

Alias específico do OpenChamber para escolher a distro WSL. `CODEX_WSL_DISTRO` tem prioridade quando ambas estão definidas.
