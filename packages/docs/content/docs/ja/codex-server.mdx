---
title: Codex サーバー
description: OpenChamber をローカルまたはリモートの Codex サーバーに接続します。
---

# Codex サーバー

OpenChamber は Codex サーバーの上で動作します。デフォルトでは OpenChamber がサーバーを起動するため、何もする必要はありません。このページが必要なのは、すでに実行しているサーバーに OpenChamber を向けたい場合、または OpenChamber が起動するサーバーを管理したい場合だけです。
...
サーバーの起動中または再起動中、OpenChamber は "Codex is restarting" 状態を表示し、準備が整うまでリクエストを一時停止します。これは起動直後や再起動直後には正常です。いつまでも消えない場合は、[Codex 接続](/troubleshooting/codex-connection/) を参照してください。

## 関連

- [プロバイダー、モデル、エージェント](/providers/) — サーバーの接続先を設定する
- [Codex 接続](/troubleshooting/codex-connection/) — 接続できない場合
