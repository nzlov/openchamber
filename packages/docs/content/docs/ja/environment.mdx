---
title: 環境変数
description: 環境変数で OpenChamber と Codex 連携を設定します。
---
...
OpenChamber が実行する `codex` 実行ファイルへのパスです。

### `CODEX_CONFIG`

特定の Codex 設定ファイルへのパスです。

### `CODEX_CONFIG_DIR`

エージェント、スキル、スニペット、設定検出に使う特定の Codex 設定ディレクトリへのパスです。

### `CODEX_DATA_DIR`

管理対象 Codex サーバーのカスタムデータディレクトリです。

### `CODEX_WSL_DISTRO`

Windows 上の Codex 連携に使う WSL ディストリビューションを選びます。

### `OPENCHAMBER_CODEX_WSL_DISTRO`

WSL ディストリビューション選択用の OpenChamber 固有エイリアスです。両方が設定されている場合は `CODEX_WSL_DISTRO` が優先されます。
