...
Provider sign-ins are stored by Codex, not OpenChamber, so they're shared with the Codex CLI. If you set the same provider in more than one place, the most specific setting wins: a per-project setting overrides your personal one.
