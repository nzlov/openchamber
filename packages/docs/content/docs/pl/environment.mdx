...
description: Skonfiguruj integrację OpenChamber i Codex za pomocą zmiennych środowiskowych.
---
...
Ścieżka do executable `codex`, którego ma używać OpenChamber.

### `CODEX_CONFIG`

Ścieżka do konkretnego pliku config Codex.

### `CODEX_CONFIG_DIR`

Ścieżka do katalogu config Codex dla agents, skills, snippets i config discovery.

### `CODEX_DATA_DIR`

Własny katalog danych dla managed Codex runtime.

### `CODEX_WSL_DISTRO`

Wybiera WSL distro używane do integracji Codex w Windows.

### `OPENCHAMBER_CODEX_WSL_DISTRO`

Alias specyficzny dla OpenChamber do wyboru WSL distro. `CODEX_WSL_DISTRO` ma pierwszeństwo, gdy ustawione są obie zmienne.
