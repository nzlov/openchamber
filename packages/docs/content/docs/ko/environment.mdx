---
title: 환경 변수
description: 환경 변수로 OpenChamber와 Codex 연동을 설정합니다.
---
...
OpenChamber가 실행할 `codex` executable 경로입니다.

### `CODEX_CONFIG`

특정 Codex config 파일 경로입니다.

### `CODEX_CONFIG_DIR`

agents, skills, snippets, config discovery에 사용할 Codex config 디렉터리입니다.

### `CODEX_DATA_DIR`

managed Codex runtime의 사용자 지정 데이터 디렉터리입니다.

### `CODEX_WSL_DISTRO`

Windows에서 Codex 연동에 사용할 WSL distro를 선택합니다.

### `OPENCHAMBER_CODEX_WSL_DISTRO`

WSL distro를 선택하는 OpenChamber 전용 alias입니다. 둘 다 설정하면 `CODEX_WSL_DISTRO`가 우선합니다.
